...
function createViz(elem, id, height, data, theme, options) {
  ...
  svg.selectAll('text').attr('font-size', txtSize + 'em').style('font-family', theme.typography.fontFamily.sansSerif).attr('fill', theme.colors.text).call(truncateLabel, maxTxtLength).on('mouseover', function (event, d) {
...
function matrix(data, id, height, options) {
  var theme = (0,_grafana_ui__WEBPACK_IMPORTED_MODULE_3__.useTheme)(); // some react related voodoo
