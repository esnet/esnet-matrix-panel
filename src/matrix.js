...
function createViz(elem, height, data, src, target, val, theme, cellSize, cellPadding, txtLength, txtSize) {
  ...
  .attr("transform","translate(-11.5,-12)rotate(90)")

  // Build X scales and axis:
  var y = d3.scaleBand()
    .range([ height, 0 ])
    .domain(names.slice().reverse())
    .padding(cellPadding);
  svg.append("g")
    .call(d3.axisLeft(y))
    .select(".domain").remove()
  ...
  svg.selectAll("text")
    ...
    .on('mouseover', function (event, d) {
        ...
        div.style("left",rect.left + rect.width - divSize.width / 2 + "px")
